...
// ===========================
// Before coding any data visualizations, set up the width, height and margins of the graph.
// Add room for label text as well as text padding.
...
// Set the radius function for each dot that will appear in the graph.
// Allows us to easily call it in the mobility section of our code.
var circRadius;
function crGet() {
  if (width <= 530) {
    circRadius = 5;
...
// Nest this attribute in a function helps to easily change the location of the label group
...
// Use xText to append the three text SVG files, with y coordinates specified to space out the values.
// 1. Poverty
xText
  .append("text")
  .attr("y", -26)
...
// Specify the variables like to allow us transform attributes more readable.
...
// yText allows us to select the group without excess code.
...
// ====================================
// Call a "visualize" function on the data obtained with d3's .csv method.
...
function visualize(theData) {
  ...
  // Designate the defaults here, which carry the same names as the headings in their matching .csv data file.
  var curX = "poverty";
  var curY = "obesity";

  // Save empty variables for our the min and max values of x and y.
  // Helps to alter the values in functions and remove repetitious code.
  var xMin;
  var xMax;
  var yMin;
  var yMax;

  // The function allows for the setting up tooltip rules (see d3-tip.js).
  var toolTip = d3
    .tip()
    .attr("class", "d3-tip")
    .offset([40, -60])
  ...
  // Note: D3 4.0 made this a lot less cumbersome than before. Kudos to mbostock.
  ...
  // Match labels with the circles in the map
  ...
  // =========================
  // Use d3, to resize function whenever the window dimensions change.
